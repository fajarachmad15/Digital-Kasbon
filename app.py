...
if query_id:
    ...
    if not st.session_state.mgr_logged_in:
        ...
        if st.button("Masuk & Verifikasi", type="primary", use_container_width=True):
            ...
            else: st.warning("NIK wajib 6 digit & Password min 6 karakter.")
        st.stop()
    ...
    try:
        ...
        with c2:
            ...
            st.write(f"**Terbilang:** {row_data[8]}") # TERBILANG DITAMPILKAN
...
# --- 5. LOGIKA LOGIN GOOGLE (PENGADAAN) ---
...
if st.session_state.submitted:
    ...
    if kode_store:
        try:
            ...
            st.text_input("Email Request", value=st.experimental_user.email, disabled=True)
            ...
            if st.button("Kirim", type="primary"):
                if nama_p and len(nip_p)==6 and nom_r.isdigit() and mgr_f!="-" and sc_f!="-":
                    ...
                    # NOMOR KASBON TANPA DD, RESET HARIAN
