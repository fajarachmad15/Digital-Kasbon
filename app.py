...
if query_id:
    ...
    try:
        ...
        c1, c2 = st.columns(2)
        with c1: st.write(f"**Tgl:** {row_data[0]}"); st.write(f"**Dibayarkan:** {row_data[4]} / {row_data[5]}")
        with c2: st.write(f"**Nominal:** Rp {int(row_data[7]):,}"); st.write(f"**Terbilang:** {row_data[8]}")
    ...
    except: pass # Image 70d18a: Hapus error notif
    st.stop()

if not st.experimental_user.is_logged_in:
    st.subheader("🌐 Login Akun Gmail")
    st.info("Klik tombol di bawah untuk masuk menggunakan Gmail Anda.")
    if st.button("Sign in with Google", type="primary", use_container_width=True): st.login()
    st.stop()

# --- 4. FORM PENGADAAN (PIC) ---
if st.session_state.get('submitted'):
    ...
    if kode_store:
        try:
            ...
            st.text_input("", value=st.experimental_user.email, disabled=True)
            ...
            if st.button("Kirim", type="primary"):
                if nama_p and len(nip_p)==6 and nom_r.isdigit() and mgr_f!="-" and sc_f!="-":
                    ...
                    no_p = f"KB{kode_store}-{tgl_n.strftime('%m%y')}-{str(count_today + 1).zfill(3)}"
                    final_t = terbilang(int(nom_r)) + " Rupiah"
                    sheet.append_row([tgl_n.strftime("%Y-%m-%d %H:%M:%S"), no_p, kode_store, st.experimental_user.email, nama_p, nip_p, dept, nom_r, final_t, kep, "Terlampir", janji.strftime("%d/%m/%Y"), sc_f, mgr_f, "Pending"])
                    send_email_with_attachment(mgr_f.split("(")[1][:-1], f"Approval Kasbon {no_p}", f"Data {nama_p} senilai Rp {int(nom_r):,}. <a href='{BASE_URL}?id={no_p}'>Klik Approve</a>", bukti)
