...
if st.session_state.submitted:
    ...
    if kode_store:
        try:
            ...
            # MEMUNCULKAN KEMBALI EMAIL REQUEST (Image dde323 Fix)
            ...
            if st.button("Kirim Pengajuan", type="primary"):
                if nama_p and len(nip_p)==6 and nom_r.isdigit() and mgr_f!="-" and sc_f!="-":
                    ...
                    send_email_with_attachment(mgr_f.split("(")[1][:-1], f"Approval Kasbon {no_p}", f"Data {nama_p} Rp {int(nom_r):,}. <a href='{BASE_URL}?id={no_p}'>Link</a>", bukti)
