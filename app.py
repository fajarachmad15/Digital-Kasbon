...
# --- SETTING HALAMAN & CSS (WAJIB PALING ATAS) ---
...
st.markdown("""
    ...
    .terbilang-text { font-size: 150% !important; font-weight: bold; margin-bottom: 10px; line-height: 1.5; }
    @media (prefers-color-scheme: dark) { .terbilang-text { color: #FFFFFF !important; } }
    @media (prefers-color-scheme: light) { .terbilang-text { color: #000000 !important; } }
    ...
    div[data-testid="stInputNumberStepContainer"] { display: none !important; }
    </style>
...
# --- DATABASE HEADER HELPERS (PENTING UNTUK HEADER DINAMIS) ---
class DataContext:
    ...
    def get_val(self, col_name):
        try:
            ...
            if idx != -1 and idx < len(self.row_data): return self.row_data[idx]
        except: pass
        return ""
    def update(self, col_name, value):
        try:
            col_idx = self.headers.index(col_name) + 1
            self.sheet.update_cell(self.cell.row, col_idx, value)
        except: pass
# 1. --------End_Part_1_Config_Helper-----------

# 2. --------Start_Part_2_Session_Auth----------
...
# 2. --------End_Part_2_Session_Auth-----------

# 3. --------Start_Part_3_Portal_Logic----------
...
if query_id:
    try:
        ...
        # --- BACA DATA DENGAN HEADER DINAMIS (Wajib) ---
        headers = sheet.row_values(1)
        
        # Helper untuk index kolom (Database Update)
        def get_col_idx(name):
            try: return headers.index(name) + 1 
            except: return -1

        # Helper untuk index value (Read Data)
        ...
        # Helper lokal untuk ambil data aman
        ...
        # --- MAPPING VARIABEL (Sesuai Kode Lama) ---
        ...
        try: r_nominal_awal = int(get_val("Nominal"))
    ...
    st.stop()
# 3. --------End_Part_3_Portal_Logic-----------

# 4. --------Start_Part_4_Input_Form----------
else:
    # --- FORM PENGAJUAN BARU (ELSE) ---
    if st.session_state.submitted:
        ...
        * **Janji Penyelesaian** : {d['janji']}
        ...
        # --- FITUR CEK STATUS (VISUAL STEP-BY-STEP) ---
        st.markdown("### 🕵️ Cek Status Pengajuan")
        c_cek_1, c_cek_2 = st.columns([3, 1])
        with c_cek_1:
            id_cek = st.text_input("Masukkan Nomor Pengajuan", label_visibility="collapsed", placeholder="KB...")
        with c_cek_2:
            btn_cek = st.button("Cek Status", type="primary", use_container_width=True)
            
        if btn_cek and id_cek:
            try:
                sh = get_creds(); cl = gspread.authorize(sh); ws = cl.open_by_key(SPREADSHEET_ID).worksheet("DATA_KASBON_AZKO")
                
                # Logic Header Dinamis untuk Cek Status
                h_cek = ws.row_values(1)
                def g_idx(n): 
                    try: return h_cek.index(n)
                    except: return -1
                
                try:
                    cell_cek = ws.find(id_cek)
                except:
                    st.error("❌ Nomor Pengajuan tidak ditemukan.")
                    st.stop()

                d_cek = ws.row_values(cell_cek.row)
                def g_val(n):
                    i = g_idx(n)
                    return d_cek[i] if i != -1 and i < len(d_cek) else ""
                
                # Ambil Nama Petugas untuk Display
                raw_csr = g_val("Senior_Cashier")
                c_csr_name = raw_csr.split(" - ")[1].split(" (")[0] if " - " in raw_csr else "Cashier"
                
                raw_mgr = g_val("Manager_Incharge")
                c_mgr_name = raw_mgr.split(" - ")[1].split(" (")[0] if " - " in raw_mgr else "Manager"
                
                c_req_name = g_val("Dibayarkan_Kepada") or "Requester"

                st.divider()

                # --- FUNGSI RENDER VISUAL (KEMBALI KE DESIGN LAMA) ---
                ...
                stat_mgr = g_val("Status_Approval")
                ...
                stat_csr = g_val("Status_Verifikasi")
                ...
                stat_real = g_val("Status_Realisasi")
                ...
                stat_v_real = g_val("Status_Verifikasi_Rea")
                ...
                # STEP 7: Final Cek
                stat_final = g_val("Timestamp_Fin_Cek")
                if stat_final:
        ...
        if kode_store:
            try:
                ...
                if nom_r.isdigit(): 
                    ...
                    st.markdown(f'<div class="terbilang-text">{final_t}</div>', unsafe_allow_html=True)
                else:
                    final_t = ""
                    ...
                    if valid_nama and len(nip)==6 and nom_r.isdigit() and valid_kep and dept!="-" and mgr_f!="-" and sc_f!="-":
                        try:
                            with st.spinner("Processing..."):
                                ...
                                # Menggunakan Header Dinamis untuk cari kolom ID
                                headers_temp = sheet.row_values(1)
                                try: col_id_idx = headers_temp.index("No_Pengajuan") + 1
                                except: col_id_idx = 2 # Fallback default B
                                
                                all_ids = sheet.col_values(col_id_idx)
                                last_n = 0
                                for r in all_ids:
                                    if r.startswith(prefix):
                                        try: last_n = max(last_n, int(r.split("-")[-1]))
                                        except: continue
                                ...
                                if bukti:
                                    try:
                                        ...
                                        if res.get("status") == "success":
                                            link_drive = res.get("url")
                                        else:
                                            st.error(f"Gagal Upload: {res.get('message')}"); st.stop()
                                    except Exception as e:
                                        st.error(f"Error Koneksi Upload: {e}"); st.stop()
                                
                                # --- INTI PERUBAHAN: INPUT DATA (DINAMIS DICTIONARY) ---
                                # Data disusun dalam Dictionary, bukan List urut
                                data_input = {
                                    ...
                                    # Field lain kosongkan
                                    "Status_Approval": "", "Reason_Reject_App": "", "Status_Email_App": "",
                                    "Timestamp_App": "", "NIP_App": "", 
                                    "Timestamp_Ver": "", "NIP_Ver": "", "Status_Verifikasi": "", "Reason_Reject_Ver": "", "Status_Email_Ver": "",
                                    "Timestamp_Rec": "", "NIP_Rec": "", "Status_Uang": "",
                                ...
                                # BACA HEADER SAAT INI & SUSUN ROW
                                current_headers = sheet.row_values(1)
                                row_to_submit = [data_input.get(h, "") for h in current_headers]
                                
                                # KIRIM KE DATABASE
                                sheet.append_row(row_to_submit)
                                ...
                                """
                                # Ambil email manager dari map yang sudah dibuat di atas
                                target_mgr_email = mgr_map.get(mgr_f, "")
                                if target_mgr_email:
                                    send_email_with_attachment(target_mgr_email, f"Pengajuan Kasbon {no_p}", email_body)
                            
                                st.session_state.data_ringkasan = {
                                    'no_pengajuan': no_p, 'kode_store': kode_store, 'nama': nama_p, 
                                    'nip': nip, 'dept': dept, 'nominal': nom_r, 'terbilang': final_t, 
                                    'keperluan': kep, 'janji': janji.strftime("%d/%m/%Y"),
                                    'tgl_jam': tgl_full, 'link_pendukung': link_drive 
                                }
