...
# --- 5. LOGIKA LOGIN EMAIL PIC (MANUAL TAPI PERSISTENT) ---
if not st.session_state.pic_email:
    st.subheader("📧 Login Email Kerja")
    st.info("Masukkan email kerja Anda. Cukup isi sekali untuk pengajuan seterusnya.")
    email_input = st.text_input("Email", placeholder="nama.anda@gmail.com")
    if st.button("Masuk & Simpan", type="primary", use_container_width=True):
        if "@" in email_input:
            st.session_state.pic_email = email_input; st.rerun()
...
if st.session_state.submitted:
    ...
    c1, c2 = st.columns(2)
    if c1.button("Buat Baru"): st.session_state.submitted = False; st.rerun()
    if c2.button("Ganti Email"): st.session_state.pic_email = ""; st.rerun()
