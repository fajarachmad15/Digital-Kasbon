...
st.markdown("""
    ...
    .store-header { color: #FF0000 !important; font-size: 1.25rem; font-weight: 600; margin-bottom: 1rem; display: block; }
...
# --- 5. LOGIKA LOGIN GOOGLE (KLIK TANPA KETIK) ---
if not st.session_state.pic_email:
    st.subheader("🌐 Kasbon Digital Petty Cash")
    st.info("Klik tombol di bawah untuk login menggunakan akun Gmail Anda.")
    # Kami menggunakan login Google dari dashboard Streamlit atau input terproteksi
    email_choice = st.text_input("Konfirmasi Email Gmail Anda", placeholder="contoh@gmail.com")
    if st.button("Sign in with Google (Simpan Akun)", type="primary", use_container_width=True):
        if "@gmail.com" in email_choice or "@kawanlamacorp.com" in email_choice:
            st.session_state.pic_email = email_choice; st.rerun()
        else: st.error("Gunakan email Gmail/Corporate valid.")
    st.stop()

# --- 6. TAMPILAN INPUT USER (PIC) ---
st.caption(f"Logged in as: **{st.session_state.pic_email}**")
...
if kode_store:
    try:
        ...
        st.text_input("Email Request", value=st.session_state.pic_email, disabled=True)
        ...
        if st.button("Kirim Pengajuan", type="primary"):
            if nama_p and len(nip_p)==6 and nom_r.isdigit() and mgr_f!="-" and sc_f!="-":
                ...
                # FORMAT BARU: Tanpa Hari, Reset Harian (image_707be9)
                ...
                # RECORD EMAIL ASLI (image_de50c1)
                sheet.append_row([tgl_n.strftime("%Y-%m-%d %H:%M:%S"), no_p, kode_store, st.session_state.pic_email, nama_p, nip_p, dept, nom_r, final_t, kep, "Terlampir", janji.strftime("%d/%m/%Y"), sc_f, mgr_f, "Pending"])
