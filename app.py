1--------Start_Part_Imports_Config_Helper_Functions----------
...
# --- DATABASE HEADER HELPERS (CONTEXTUAL) ---
class DataContext:
    def __init__(self, sheet, cell, headers, row_data):
        self.sheet = sheet
        self.cell = cell
    ...
    def get_val(self, col_name):
        try:
            ...
            if idx != -1 and idx < len(self.row_data):
                return self.row_data[idx]
        except: pass
        return ""

    def get_col_idx(self, col_name):
        try: return self.headers.index(col_name) + 1
        except: return -1
    
    def update(self, col_name, value):
        self.sheet.update_cell(self.cell.row, self.get_col_idx(col_name), value)
1--------End_Part_Imports_Config_Helper_Functions-----------

2--------Start_Part_Session_State_Routing_Logic_Portal_View----------
...
def render_login_section(role_key_check, role_name, assigned_str, target_nik, store_code_req):
    if not st.session_state.get(role_key_check, False):
        st.subheader(f"🔐 Verifikasi {role_name}")
        ...
        if st.button("Masuk & Verifikasi", type="primary", use_container_width=True):
            ...
            if len(v_nik) == 6 and len(v_pass) >= 6:
                ...
                if user: 
                    st.session_state[role_key_check] = True
    ...
    if st.session_state.user_store_code != store_code_req:
        st.error("⛔ AKSES DITOLAK! Store tidak sesuai.")
        st.stop()

def render_details_common(ctx, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, display_status):
    st.info(f"### Rincian Pengajuan")
    st.markdown(f"""
    * **Nomor Pengajuan Kasbon** : {st.query_params.get("id")}
    * **Tgl dan Jam Pengajuan** : {ctx.get_val("Timestamp_Req")}
...
# --- PORTAL LOGIC FUNCTIONS ---

def render_portal_manager(ctx, pic_email, target_manager_email, target_manager_nik, assigned_manager_str, assigned_cashier_str, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, status_mgr, query_id, r_req_email):
    ...
    # Login
    render_login_section('mgr_logged_in', "Manager", assigned_manager_str, target_manager_nik, ctx.get_val("Kode_Store"))

    # Details
    render_details_common(ctx, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, display_status)
    ...
    if st.session_state.user_role == "Manager":
        ...
        if b1.button("✓ APPROVE", use_container_width=True):
            ...
            ctx.update("Timestamp_App", tgl)
            ctx.update("NIP_App", st.session_state.user_nik)
            ctx.update("Status_Approval", "Approved")
            ctx.update("Reason_Reject_App", "-")
            ...
            try:
                ...
                <html><body style='font-family: Arial, sans-serif; font-size: 14px; color: #000000;'>
                    ...
                    <table style='border: none; border-collapse: collapse; width: 100%; max-width: 600px;'>
                        ...
                        <tr><td style='padding: 2px 0;'>Tgl dan Jam Pengajuan</td><td>: {ctx.get_val("Timestamp_Req")}</td></tr>
                ...
                if send_email_with_attachment(target_cashier_email, f"Verifikasi Kasbon {query_id}", email_msg):
                    ctx.update("Status_Email_App", "Tekirim ke Cashier")
            except: pass
        ...
        if b2.button("✕ REJECT", use_container_width=True):
            ...
            ctx.update("Timestamp_App", tgl)
            ctx.update("NIP_App", st.session_state.user_nik)
            ctx.update("Status_Approval", "Reject")
            ctx.update("Reason_Reject_App", alasan)
...
def render_portal_cashier(ctx, pic_email, target_cashier_email, target_cashier_nik, assigned_cashier_str, target_manager_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, status_cashier, query_id, r_req_email):
    ...
    render_login_section('mgr_logged_in', "Cashier", assigned_cashier_str, target_cashier_nik, ctx.get_val("Kode_Store"))
    ...
    if st.session_state.user_role == "Senior Cashier":
        ...
        if k1.button("✓ VERIFIKASI APPROVE", use_container_width=True):
            tgl = datetime.datetime.now(WIB).strftime("%Y-%m-%d %H:%M:%S")
            ctx.update("Timestamp_Ver", tgl)
            ctx.update("NIP_Ver", st.session_state.user_nik)
            ctx.update("Status_Verifikasi", "Verifikasi Approved")
            ctx.update("Reason_Reject_Ver", "-")
            ...
            try:
                ...
                if send_email_with_attachment(r_req_email, f"Kasbon Disetujui {query_id}", email_req_body):
                        ctx.update("Status_Email_Ver", "Tekirim ke Pemohon")
            except: pass
        ...
        if k2.button("✕ VERIFIKASI REJECT", use_container_width=True):
            ...
            ctx.update("Timestamp_Ver", tgl)
            ctx.update("NIP_Ver", st.session_state.user_nik)
            ctx.update("Status_Verifikasi", "Verifikasi Reject")
            ctx.update("Reason_Reject_Ver", alasan_c)
...
def render_portal_confirmation(ctx, pic_email, r_req_email, target_cashier_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, query_id):
    ...
    if st.button("Konfirmasi uang sudah diterima dan sesuai", type="primary", use_container_width=True):
        ...
        ctx.update("Timestamp_Rec", tgl_terima)
        ctx.update("NIP_Rec", r_nip)
        ctx.update("Status_Uang", "Sudah diterima")
...
def render_portal_realization_input(ctx, pic_email, r_req_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, query_id):
    ...
    if st.button("Kirim Laporan Realisasi", type="primary", use_container_width=True):
        if bukti_real and uang_digunakan >= 0:
            try:
                ...
                ctx.update("Timestamp_Rea", tgl_real)
                ctx.update("NIP_Rea", r_nip)
                ctx.update("Nominal_Pembelanjaan_Rea", uang_digunakan)
                ctx.update("Terbilang_Rea1", terbilang_guna)
                ctx.update("Uang_Dikembalikan_Rea", val_kembali)
                ctx.update("Terbilang_Rea2", txt_kembali)
                ctx.update("Uang_Diterima_Rea", val_terima)
                ctx.update("Terbilang_Rea3", txt_terima)
                ctx.update("Bukti_Lampiran_Rea", link_bukti)
                ctx.update("Status_Realisasi", "Terrealisasi")
                ctx.update("Status_Email_Rea", "Tekirim ke Mgr")
...
def render_portal_realization_verification(ctx, pic_email, target_cashier_email, target_cashier_nik, assigned_cashier_str, assigned_manager_str, target_manager_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, status_verif_real, query_id, r_req_email, link_real_lampiran):
    ...
    # Login Cashier Logic Check (Not using general render_login because needs specific Role check for Realization)
    ...
    * **Janji Penyelesaian** : {r_janji}
    ...
    try: u_kembali_db = int(ctx.get_val("Uang_Dikembalikan_Rea"))
    except: u_kembali_db = 0
    
    try: u_terima_db = int(ctx.get_val("Uang_Diterima_Rea"))
    ...
    if st.button("Submit Verifikasi", type="primary"):
        ...
        ctx.update("Timestamp_Ver_Rea", tgl_verif)
        ctx.update("NIP_Ver_Rea", st.session_state.user_nik)
        ctx.update("Uang_Dikembalikan_Ver_Rea", final_u_kembali)
        ctx.update("Terbilang_Ver_Rea1", txt_kembali_final)
        ctx.update("Uang_Diterima_Ver_Rea", final_u_terima)
        ctx.update("Terbilang_Ver_Rea2", txt_terima_final)
        ctx.update("Status_Verifikasi_Rea", status_pilihan)
        ctx.update("Reason", reason_verif)
...
def render_portal_final_check(ctx, pic_email, target_manager_email, target_manager_nik, assigned_manager_str, target_cashier_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, query_id, link_real_lampiran):
    ...
    # Specific Manager Login for Final Check
    ...
    * **Janji Penyelesaian** : {r_janji}
    ...
    if st.button("Posting", type="primary"):
        ...
        ctx.update("Timestamp_Fin_Cek", tgl_final)
        ctx.update("NIP_Fin_Cek", st.session_state.user_nik)
        ctx.update("Status_Qty_Value_Nota", q1_ans)
        ctx.update("Reason_Fin_Cek_1", q1_reason)
        ctx.update("Status_Item", q2_ans)
        ctx.update("Reason_Fin_Cek_2", q2_reason)
        ctx.update("Revisi_Bukti_Realisasi", link_revisi)
...
def render_portal_completed(ctx, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, query_id, link_real_lampiran):
...
def execute_portal_logic(query_id, pic_email):
    try:
        creds = get_creds()
        client = gspread.authorize(creds)
        ...
        # Initialize Context
        ctx = DataContext(sheet, cell, headers, row_data)
        
        # Mapping Data Dasar
        r_store_code = ctx.get_val("Kode_Store")
        r_no = ctx.get_val("No_Pengajuan")
        r_req_email = ctx.get_val("Email_Request")
        r_nama = ctx.get_val("Dibayarkan_Kepada")
        r_nip = str(ctx.get_val("NIP_Req"))
        r_dept = ctx.get_val("Departemen")
        
        try: r_nominal_awal = int(ctx.get_val("Nominal"))
        except: r_nominal_awal = 0
            
        r_terbilang_awal = ctx.get_val("Terbilang_Req")
        r_keperluan = ctx.get_val("Keperluan")
        r_link_lampiran = ctx.get_val("Bukti_Lampiran_Req")
        r_janji = ctx.get_val("Janji_Penyelesaian")
        
        assigned_cashier_str = ctx.get_val("Senior_Cashier")
        assigned_manager_str = ctx.get_val("Manager_Incharge")
        ...
        # Statuses
        status_mgr = ctx.get_val("Status_Approval")
        reason_mgr = ctx.get_val("Reason_Reject_App")
        status_cashier = ctx.get_val("Status_Verifikasi")
        reason_csr = ctx.get_val("Reason_Reject_Ver")
        status_terima = ctx.get_val("Status_Uang")
        status_real = ctx.get_val("Status_Realisasi")
        link_real_lampiran = ctx.get_val("Bukti_Lampiran_Rea") if ctx.get_val("Bukti_Lampiran_Rea") else "-"
        status_verif_real = ctx.get_val("Status_Verifikasi_Rea")
        final_cek_timestamp = ctx.get_val("Timestamp_Fin_Cek")

        # --- ROUTING ---
        if status_mgr in ["", "Pending"]:
            render_portal_manager(ctx, pic_email, target_manager_email, target_manager_nik, assigned_manager_str, assigned_cashier_str, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, status_mgr, query_id, r_req_email)
        
        elif status_mgr == "Approved" and status_cashier in ["", "Pending"]:
            render_portal_cashier(ctx, pic_email, target_cashier_email, target_cashier_nik, assigned_cashier_str, target_manager_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, status_cashier, query_id, r_req_email)
        
        elif status_cashier == "Verifikasi Approved" and status_terima in ["", "Pending"]:
            render_portal_confirmation(ctx, pic_email, r_req_email, target_cashier_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, query_id)
        
        elif status_terima == "Sudah diterima" and status_real in ["", "Pending"]:
            render_portal_realization_input(ctx, pic_email, r_req_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, query_id)
        
        elif status_real == "Terrealisasi" and status_verif_real in ["", "Pending"]:
            render_portal_realization_verification(ctx, pic_email, target_cashier_email, target_cashier_nik, assigned_cashier_str, assigned_manager_str, target_manager_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, status_verif_real, query_id, r_req_email, link_real_lampiran)
        
        elif (status_verif_real == "Ya, Sesuai" or status_verif_real == "Tidak Sesuai") and final_cek_timestamp == "":
            render_portal_final_check(ctx, pic_email, target_manager_email, target_manager_nik, assigned_manager_str, target_cashier_email, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, query_id, link_real_lampiran)
        
        elif final_cek_timestamp != "":
            render_portal_completed(ctx, r_nama, r_nip, r_dept, r_nominal_awal, r_terbilang_awal, r_keperluan, r_link_lampiran, r_janji, query_id, link_real_lampiran)
    ...
    except Exception as e: st.error(f"Error Database/System: {e}"); st.stop()
2--------End_Part_Session_State_Routing_Logic_Portal_View-----------

3--------Start_Part_Submission_Form_Logic_User_View----------
def render_status_check_section():
...
def render_submission_form(pic_email):
...
3--------End_Part_Submission_Form_Logic_User_View-----------

4--------Start_Part_Main_Execution----------
# --- SETTING HALAMAN & CSS ---
...
if query_id:
    execute_portal_logic(query_id, pic_email)
else:
    render_submission_form(pic_email)
4--------End_Part_Main_Execution-----------